...
- **Email / password** — only active when Supabase environment variables are present. If they are missing, the app shows a **"Demo mode active"** banner, disables the email fields, and points to the setup steps below instead of crashing.
...
- The header's "Browser status / Force online / Force offline" selector lets you demo offline behaviour without disconnecting your machine.
...
Testers cannot reach the transcript/QC stage without either:
...
- `corrected_transcript_text` — optional tester/QC edit. Saved separately; the transcript screen always shows **"Original transcript preserved"** alongside it.
...
- `edited_by_user` / `requires_qc_verification` — set together whenever a field is hand-edited; QC review shows an **"Edited — verify in QC"** badge.
