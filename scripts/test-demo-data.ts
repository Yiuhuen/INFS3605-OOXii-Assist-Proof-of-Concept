...
// A. Exactly 12 records, all tagged as demo data, with unique IDs.
cases.push({
  name: "A. seedDemoRecords returns exactly 12 uniquely-identified, tagged records",
  run: () => {
    const records = seedDemoRecords();
    const clientIds = new Set(records.map((r) => r.client_id));
    const recordIds = new Set(records.map((r) => r.id));
    return (
      expectEqual("record count", records.length, 12) ??
      ...
      expectEqual("unique anonymous_client_id count", clientIds.size, 12) ??
      expectEqual("unique record_id count", recordIds.size, 12) ??
...
// final report rather than silently forced to match.
cases.push({
  ...
  run: () => {
    ...
    return (
      expectEqual("total records", summary.totalRecords, 12) ??
      expectEqual("needs QC count", summary.needsQcCount, 7) ??
...
// M. QC screen filters actually populate: Needs QC (7), Edited (1), Missing (4 records with a REQUIRED_EXTRACTED_FIELDS gap: R3, R4, R7, R11).
cases.push({
  name: "M. QC filter tabs populate as expected",
  run: () => {
    const records = seedDemoRecords();
    return (
      expectEqual("needs_qc filter count", filterRecords(records, "needs_qc").length, 7) ??
...
cases.push({
  ...
  run: () => {
    ...
    return (
      ...
      expectEqual("longlist row count (header + 12 records)", longlistLines.length, 13) ??
      expectEqual("audit row count (header + 12 records x 8 fields)", auditLines.length, 1 + 12 * 8)
    );
  }
});
